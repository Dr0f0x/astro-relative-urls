...
import {
  ...
  RelativeUrlConfiguration,
} from './configuration'
import { AstroIntegrationLogger } from 'astro'

export default function rewriteLinksAndAssets(
  ...
  return {
    ...
    hooks: {
      'astro:build:done': async ({
        ...
        console.log(
          chalk.bgBlue.black(' running post-build link and asset rewrites '),
        )
        ...
        for (let i = 0; i < htmlFiles.length; i++) {
          ...
          if (
            ...
            config.pageLinkAttributesToChange.length > 0
          ) {
          ...
          if (
            ...
            config.assetAttributesToChange.length > 0
          ) {
